...
-- Пример выполнения
-- ==================================================
-- create Procedure [dbo].[hla3_Hash_create] as Begin declare @i int; end
-- Grant execute on [hla3_Hash_create] to public
-- ==================================================
Alter Procedure [dbo].[hla3_Hash_create]
As
Begin

    Print '=================================================='
    Print '*** [hla3_Hash_create]'
    ...
    Begin
        Select @max_n=Max(len(ue.uexon_seq))
            From hla3_uexon ue
        Truncate Table hla3_uexon_part
        ...
        -- Данные по экзонам
        Update hla3_uexon_part
            Set uexon_len       = ue.uexon_len
                ,epart_cnt      = ue.epart_cnt
                ,k_forward_back = ue.k_forward_back
            From hla3_uexon_part ep
            Inner Join hla3_uexon ue With (Nolock) On ue.uexon_iid=ep.uexon_iid

        -- Таблица уникальных частей
        Truncate Table hla3_uexon_upart
        Insert INTO hla3_uexon_upart (
             epart_hash
            ,epart_seq
            )   
            Select distinct 
                     ep.epart_hash
                    ,''
                From hla3_uexon_part ep With (Nolock)
                    Inner Join dna2_hla.dbo.hla3_uexon ue With (Nolock) On ue.uexon_iid=ep.uexon_iid
                Where 1=1
                    --And (
        ...
        Update hla3_uexon_upart
            Set epart_seq=dbo.sql_str_hash_decode(epart_hash)

        -- Таблица уникальных частей
        -- сточностью до гена и экзона
        Truncate Table hla3_uexon_ugpart
        Insert INTO hla3_uexon_ugpart (
             epart_hash
            ,gen_cd
            ,uexon_num
            ,epart_seq
        ...
        Update hla3_uexon_ugpart
            Set epart_seq=dbo.sql_str_hash_decode(epart_hash)

        -- Ключ принадлежности к разным экзонам/генам
        Update dna2_hla.dbo.hla3_uexon_ugpart
    	    Set    k_dbl = 0
        Update dna2_hla.dbo.hla3_uexon_ugpart
    	    Set    k_dbl = 1
    	    From   dna2_hla.dbo.hla3_uexon_ugpart gp
    	    Where  gp.epart_hash In (Select up.epart_hash
    	                                 From dna2_hla.dbo.hla3_uexon_ugpart up
    ...
    Begin
        Select @max_n=Max(len(ue.uexon_seq))
            From hla3_uexon ue
        Truncate Table hla3_uexon_part2
